...
const tempUpload=require('../models/PresTempUpload');
...
const storage=multer.diskStorage({
  ...
  // post method
router.post("/Tempadmin", upload.array('uploaded_Image', 10), async (req, res) => {
    try {
     ...
     for(let i=0;i<fileInArray.length;i++){
       ...
       if(fileext=="pdf")
       pdff = await cloudinary.uploader.upload(`${path.resolve(__dirname, "../templates")}/${fileInArray[i][0]}`,{ pages: true });
     }
   
      let user = new tempUpload({
        name: req.body.name,
  ...
  //get method
  router.get("/Tempadmin", async (req, res) => {
    try {
      let user = await tempUpload.find();
      res.json(user);
    } catch (err) {
      console.log(err);
    }
